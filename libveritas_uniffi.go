...
func uniffiCheckChecksums() {
	...
	{
		checksum := rustCall(func(_uniffiStatus *C.RustCallStatus) C.uint16_t {
			return C.uniffi_libveritas_uniffi_checksum_method_verifiedmessage_root_id()
		})
		if checksum != 5376 {
			// If this happens try cleaning and rebuilding your project
			panic("libveritas_uniffi: uniffi_libveritas_uniffi_checksum_method_verifiedmessage_root_id: UniFFI API checksum mismatch")
		}
	}
	{
		checksum := rustCall(func(_uniffiStatus *C.RustCallStatus) C.uint16_t {
...
func (_self *VerifiedMessage) RootId() []byte {
	_pointer := _self.ffiObject.incrementPointer("*VerifiedMessage")
	defer _self.ffiObject.decrementPointer()
	return FfiConverterBytesINSTANCE.Lift(rustCall(func(_uniffiStatus *C.RustCallStatus) RustBufferI {
		return GoRustBuffer{
			inner: C.uniffi_libveritas_uniffi_fn_method_verifiedmessage_root_id(
				_pointer, _uniffiStatus),
		}
	}))
}
