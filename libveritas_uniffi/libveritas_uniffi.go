...
type VeritasInterface interface {
	...
	VerifyWithOptions(ctx *QueryContext, msg *Message, expandNames bool, devMode bool) (*VerifiedMessage, error)
}
type Veritas struct {
	ffiObject FfiObject
}
...
// Verify a message with explicit options.
func (_self *Veritas) VerifyWithOptions(ctx *QueryContext, msg *Message, expandNames bool, devMode bool) (*VerifiedMessage, error) {
	_pointer := _self.ffiObject.incrementPointer("*Veritas")
	defer _self.ffiObject.decrementPointer()
	_uniffiRV, _uniffiErr := rustCallWithError[VeritasError](FfiConverterVeritasError{}, func(_uniffiStatus *C.RustCallStatus) unsafe.Pointer {
		return C.uniffi_libveritas_uniffi_fn_method_veritas_verify_with_options(
			_pointer, FfiConverterQueryContextINSTANCE.Lower(ctx), FfiConverterMessageINSTANCE.Lower(msg), FfiConverterBoolINSTANCE.Lower(expandNames), FfiConverterBoolINSTANCE.Lower(devMode), _uniffiStatus)
	})
	if _uniffiErr != nil {
		var _uniffiDefaultValue *VerifiedMessage
		return _uniffiDefaultValue, _uniffiErr
